...
### 11/04/2010 jgrant Modify to use curl to ftps to ATPCo.
###                   Eliminate call to 'ftp_check' routine because failures
...
then
  ...
  if [[ ! -s $OUTBOUND_WORK/$fare_file ]]
  then
    # no new fare file, no saved fare file...  error!!!
    # Log messages to stdlist with explanation
    echo -e "\nERROR: build_fare_file: No new or saved fare file found (in $OUTBOUND or $OUTBOUND_WORK)"
   
    # Log messages to logfile (for email) 
    echo -e "\nERROR: build_fare_file: No new or saved fare file found (in $OUTBOUND or $OUTBOUND_WORK)" >> $ftp_log 
    echo -e "\nPRODUCTION SUPPORT: Consult PSMC Operations Guides for AGAPPS RMAPSEND* for further information. ">> $ftp_log 

    # Nothing to save, Send email saying we'll retry to send, exit
    error_exit "NULL"
  else
    # Log message for stdlist and email
    echo -e "No new fare file found, sending saved file from Working directory."
    echo -e "No new fare file found, sending saved file from Working directory." >> $ftp_log
  fi
fi ## $fare_file present in $OUTBOUND?
}
