...
### 07/12/2016 mramsay ATPCO's DD&S project required the following changes:
###			1)  In initialize_log_file()check for a file '*FARE'. Instead of
###                     differently named files for dom, int and arb data all three
###                     data types will share the same filename.
###                   	2)  In build_fare_file()
###**************************************************************************
#####################
#####################
#####################  CONFIGURED FOR PROD
...
if [[ $fare_file = *"FARES" ]]
then
  tws_schedule="RMAPDIST"
  tws_job="RMAPSENDFARE_FTP"
  fare_type="Fares"
fi
...
then
  ...
  # Because all data types share the same filename check if there is no file present.  
  # Having no file present is permissible.  
  if [[ ! -s $OUTBOUND/$fare_file ]]
  then
